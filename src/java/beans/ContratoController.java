...
import facade.ContratoJpaController;
...
public class ContratoController implements Serializable {
    ...
    public Contrato getFiltered() {
        if (filtered == null) {
            filtered = new Contrato();
            selectedItemIndex = -1;
        }
        return filtered;
    }
    ...
    private ContratoJpaController getJpaController() {
        if (jpaController == null) {
            jpaController = new ContratoJpaController(Persistence.createEntityManagerFactory("sisplatPU"));
        }
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(pageSize) {
                ...
                public int getItemsCount() {
                    return getJpaController().getContratoCount(filtered);
                }

                @Override
                public DataModel createPageDataModel() {
                    if (current == null) {
                        return new ListDataModel(getJpaController().findContratoEntities(getPageSize(), getPageFirstItem()));
                    }
                    return new ListDataModel(getJpaController().findContratoFilter(filtered, getPageSize(), getPageFirstItem()));
                }
            };
        }
        return pagination;
    ...
    public void create() {
        try {
            ...
            JsfUtil.addSuccessMessage(FacesContext.getCurrentInstance().getApplication().getResourceBundle(context, "bundle").getString("ContratoCreated"));
            clearFilter();
            recreateModel();
        } catch (Exception e) {
            FacesContext context = FacesContext.getCurrentInstance();
    ...
    public void update() {
        try {
            ...
            FacesContext context = FacesContext.getCurrentInstance();
            JsfUtil.addSuccessMessage(FacesContext.getCurrentInstance().getApplication().getResourceBundle(context, "bundle").getString("ContratoUpdated"));
        } catch (Exception e) {
            FacesContext context = FacesContext.getCurrentInstance();
            JsfUtil.addErrorMessage(e, FacesContext.getCurrentInstance().getApplication().getResourceBundle(context, "bundle").getString("PersistenceErrorOccured"));
        }
    ...
    private void performDestroy() {
        try {
            ...
            FacesContext context = FacesContext.getCurrentInstance();
            JsfUtil.addSuccessMessage(FacesContext.getCurrentInstance().getApplication().getResourceBundle(context, "bundle").getString("ContratoDeleted"));
        } catch (Exception e) {
            FacesContext context = FacesContext.getCurrentInstance();
            JsfUtil.addErrorMessage(e, FacesContext.getCurrentInstance().getApplication().getResourceBundle(context, "bundle").getString("PersistenceErrorOccured"));
        }
    ...
    private void updateCurrentItem() {
        int count = getJpaController().getContratoCount(filtered);
        ...
        if (selectedItemIndex >= 0) {
            current = getJpaController().findContratoEntities(1, selectedItemIndex).get(0);
        }
    }
